...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({username: user.username, id: user.id}, process.env.TOKEN_SECRET,{expiresIn: '20s'});
    const refreshToken = jwt.sign({username: user.username, id: user.id}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: '120s'});
...
exports.refresh_token = async (req, res) => {
  try {
    ...
    jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
      if (!err) {
        const accessToken = jwt.sign({username: user.username, id: user.id}, process.env.TOKEN_SECRET,{expiresIn: '20s'});
